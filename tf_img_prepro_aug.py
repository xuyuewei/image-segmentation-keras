...
def shift_img(input_imgs, width_shift_range, height_shift_range,img_size):
...
def rot_randomangle(input_imgs, angle):
    if angle:
        ...
        for i in range(len(input_imgs)):
            input_imgs[i] = tf.contrib.image.rotate(input_imgs[i],random_angle)
    return input_imgs
...
def augmentation(input_imgs,
    ...
    for i in range(len(input_imgs)):
        input_imgs[i] = projective_random_transform(input_imgs[i],projective_transform_angle,img_size)
        input_imgs[i] = shift_img(input_imgs[i], width_shift_range, height_shift_range,img_size)
